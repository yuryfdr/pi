...
struct item_finder : public std::unary_function<bool,PBListBoxItem*>{
  ...
  item_finder(const std::string& s):what(s){}
  bool operator()(PBListBoxItem* item){
    //printf("%s\n",__PRETTY_FUNCTION__);
    if(item->getText().find(what)!=std::string::npos)return true;
    return false;
  }
};
...
void SearchDialog::onFind(PBWidget*){
  int in=-1;
  if(backward)
    in=text->findItemRev(start_pos,item_finder(inp_find.getText()));
  else
    in=text->findItem(start_pos,item_finder(inp_find.getText()));
  //printf("line:%d",in);
  if(in>=0){
...
SearchDialog::SearchDialog(const std::string& nm,PBTextDisplay* lb,int from,const bool replace):PBDialog(nm),text(lb),
    lb_find("Search:",this),lb_repl("Replace",this),inp_find("",this),inp_repl("",this),
    bt_repl("Replace",this),bt_find(((replace)?"Replace":"Find"),this),bt_close("Close",this),
