...
PBTextDisplay::PBTextDisplay(const std::string& str,PBWidget* par): 
  PBListBox(str,par),changed(false),sh_ln(false){}
...
void PBTextDisplay::editItem(bool ins){
  if(!item)return;
  std::string istr;
  if(ins) istr = copy_buf;
  else istr = item->getText();
  curop.str=istr;
...
/*  void fopen_hndl(PBFileChooser*fc,bool s){
  ...
  static void addRecent(const char* s){
    for(int i=0;i<9;++i){
      if(rsntFile[i].text && strcmp(rsntFile[i].text,s)==0){
        if(i!=0)std::swap(rsntFile[i].text,rsntFile[0].text);
        return;
      }
    }
    if(rsntFile[9].text)free(rsntFile[9].text);
    for(int i=8;i>=0;--i){
      if(rsntFile[i].text){
  ...
  void goto_ln_hndl(PBNumericSelector*ns,bool ok){
    if(ok){
      int ln = ns->selected();
      if(ln==0)ms->text.selectItem(0);
      else if(ln==-1)ms->text.selectItem(ms->text.itemsCount()-1);
      ms->text.selectItem(ln-1);
      ms->update();
    }
  }
  ...
  static void HandleMainMenuItem(int index){
    ...
    switch(index){
      ...
      case ITM_FONT:{
        ...
        if(str.str().find(".ttf")==std::string::npos)str<<".ttf";
      ...
      case ITM_EXIT:
        ...
        break;
      case ITM_OPEN:{
        if(ms->fileName.empty())getcwd(buff,1023);
        else strcpy(buff,ms->fileName.c_str());
      ...
      case ITM_SAVE:{
        ...
        break;}
      case ITM_ABOUT:
        Message(ICON_INFORMATION, "π", "π(pi) - Pocketbook edItor v 0.9.2\n"
  ...
  text("ta",this),sd(NULL)
  {
    ...
    }else text.addItem("Long OK or Long tap for menu");
  }
  ...
  if(argc==2){
    ...
    for(int i=1;i<argc;i++){
      if(strcmp(argv[i], "-l")==0 && i<argc-1){
        lnno=atoi(argv[i+1]); ++i;
      } else if(strcmp(argv[i], "-f")==0 && i<argc-1){
        fname = argv[i+1]; ++i;
      } else if(strcmp(argv[i], "-s")==0){
        select_mode=true;
      }
      if(i==argc-1 && !fname) fname = argv[i];
    }
  }
